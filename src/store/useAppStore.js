 function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } }import { create } from 'zustand';
...
const calculatePaymentStatus = (totalOwed, totalPaid) => {
      ...
      updateMasterCustomer: (id, updates) => set((state) => ({
        ...
        tickets: state.tickets.map((t) =>
          t.masterCustomerId === id
            ? { 
                ...t, 
                callNotes: _nullishCoalesce(updates.callNotes, () => ( t.callNotes)),
                lastUpdated: new Date() 
              }
            : t
        ),
